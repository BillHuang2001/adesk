...
- `LaunchEnv { wayland_display: Option<String>, xdg_runtime_dir: Option<String>, extra: Vec<(String, String)>, removals: Vec<String> }` + `new`, `with_wayland_display`, `with_xdg_runtime_dir`, `with_var`, `without(key)`, `overrides() -> Vec<(String, String)>`, `removals() -> &[String]`.
- `SpawnCommand { program, args, env, env_remove }` + `new`, `with_arg`, `with_env`, `with_env_remove`; `SpawnedProcess { pid }` + `new`.
...
- `CommandSpawner` runs the program directly (never through a shell), inherits the parent environment, deletes `SpawnCommand::env_remove` and applies `SpawnCommand::env` on top (an override of a removed key wins), returns the child pid, and does not wait/reap — the server owns reaping (see below). Command construction is testable via a private `build_command` helper without spawning.
...
- `WAYLAND_DISPLAY`/`XDG_RUNTIME_DIR` must be set on the launched process (build them into `LaunchEnv` from the compositor's socket), otherwise GUI clients cannot connect. A variable the runtime must not leak into the child (e.g. a host `DISPLAY` that would make a toolkit prefer X11) is deleted with `LaunchEnv::without(key)`.

## Test Strategy

- Unit tests live inline (`#[cfg(test)] mod tests`) for pure logic with no integration counterpart: `xdg::search_dirs_with`, `terminal` env splitting, `try_exec` PATH search, `launch::LaunchEnv` (`overrides`/`removals` and `build_command` env layering incl. a spawned-child probe that a removed variable is gone while an untouched one is inherited), and the registry's pure-logic pieces (option builders, `Send + Sync`).
- Integration tests live in `./tests/`, one file per concern, sharing `./tests/support/mod.rs` (recording mock `ProcessSpawner`, fake `Clock`, tempdir `.desktop` fixture writer, `AppInfo`/`LaunchRecord`/`WindowCandidate` builders):
  ...
  - `tests/registry.rs` (19) — tempdir scan/list/get, recursive subdirs, hidden/no_display filtering + `include_hidden`, precedence shadowing, query matching, launch argv/terminal/TryExec/env + `LaunchEnv` removal forwarding with the mock spawner, launch error paths (unknown app, no Exec, DBus without Exec, invalid Exec, spawn failure), monotonic launch ids, `started_at_ms` from the fake clock.
...
- No test needs a display, GPU, network, or an installed application; fixtures are written to `tempfile::tempdir()`. Two tests do a real OS spawn: `launch::command_spawner_reports_missing_program_as_io_error` (spawns a guaranteed-missing path, asserts the ENOENT `SpawnError::Io`) and `launch::spawned_child_loses_removed_variables_and_keeps_inherited_ones`, which runs `/bin/sh` (the POSIX shell of the Linux platforms this crate targets) to prove an `env_remove`d variable is absent from the child while an untouched one is inherited.
...
- The remaining 61 lib tests live inline in the modules with no integration counterpart (`clock` 4, `launch` 19, `registry` 2, `terminal` 14, `try_exec` 11, `xdg` 11) and are the sole coverage for those modules.
...
- Validation (always through the dev shell): `./scripts/dev.sh cargo test -p adesk-app-registry` (61 lib + 113 integration + 1 doctest = 175 passing, 0 ignored), `./scripts/dev.sh cargo clippy -p adesk-app-registry --all-targets --no-deps -- -D warnings` (clean), `./scripts/dev.sh cargo fmt --all --check` (clean workspace-wide), `./scripts/dev.sh cargo check --workspace --all-targets` (clean).
...
- The child's environment is **inherited, then adjusted**: `build_command` uses `std::process::Command` with no `env_clear`/`current_dir`, so every variable of the `adesk-server` process is passed through unless it is listed in `LaunchEnv::removals()` (deleted) or set by `LaunchEnv::overrides()`. A `None` `LaunchEnv` field sets nothing, and an override of a removed key wins because removals are applied first. The crate never names `DISPLAY`, has no X11/DBus special-casing, and `terminal` wrapping leaves the environment untouched (it feeds `env`/`env_remove`-empty `SpawnCommand`s that `launch` then fills).
- `adesk-testkit` provides `.desktop` fixtures (`FixtureDir`, `TestApp`) and a helper process for end-to-end tests; this crate's own tests use `tempfile`, and apart from the missing-program spawn error case only the `/bin/sh` env-removal probe touches a real process.
...
- `cargo test -p adesk-app-registry`: 61 lib + 113 integration + 1 doctest = 175 passing, 0 ignored.
