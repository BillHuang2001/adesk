...
- The child's environment is **inherited, not replaced**: `build_command` uses `std::process::Command` with no `env_clear`/`env_remove`/`current_dir`, so only the keys in `LaunchEnv::overrides()` are set/overridden and every other variable (incl. `DISPLAY`) is passed through from the `adesk-server` process unchanged. A `None` `LaunchEnv` field sets nothing — it does not unset the parent's value. The crate never names `DISPLAY`, has no X11/DBus special-casing, and `terminal` wrapping leaves the environment untouched (it feeds `env`-empty `SpawnCommand`s that `launch` then fills).
