...
- Layering: the 39 unit tests drive `policy::*`/`WindowModel` directly; `tests/policy_matrix.rs` is the *only* exercise of the `WindowManager` façade in `manager.rs` (outside the `lib.rs` doctest), so the two layers are not interchangeable.
- Redundancy to be aware of when trimming: the integration assertions re-verify policy semantics already covered unit-side — `late_app_id_change_flows_into_window_info_and_list_windows` ≈ `app_id_changes_update_the_record_and_return_no_actions` + `app_id_change_on_an_unknown_id_is_ignored`; `destroyed_surface_key_gets_a_fresh_id_when_remapped` ≈ `ids_are_monotonic_and_never_reused_across_destroy_and_remap`; `mru_fallback_prefers_the_most_recently_used_window` ≈ `destroying_the_active_window_activates_the_mru_fallback`; and `full_lifecycle_through_the_public_api` (121 lines) re-covers map/activate/unknown-id/read-model/resolve/resize/destroy.
- Unit-side near-duplicates: `every_event_path_tolerates_unknown_ids` subsumes `destroying_an_unknown_window_is_ignored` and `activate_unknown_window_returns_an_empty_list`; `resolve_position_matches_the_documented_non_zero_origin_examples` and `resolve_position_translates_a_window_offset_by_its_geometry_origin` assert the same rules on different rects; `window_info_projects_every_field` overlaps `model::tests::record_projects_every_field_to_window_info`.
