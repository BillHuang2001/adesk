...
- **Event demux is local.** AGP event frames carry no subscription id, so the reader task publishes every event to one `tokio::sync::broadcast` channel per connection and each stream applies its own `EventFilter`. Server-side filtering is an optimisation, not a correctness dependency.
- **Backpressure is explicit.** Lagging subscribers get `ClientError::Lagged{skipped}` from `broadcast` rather than stalling the reader; the stream stays usable and the consumer re-synchronises (e.g. `list_windows`).
