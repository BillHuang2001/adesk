...
It implements the **client side of `docs/protocol.md` (AGP v1)** and nothing else: typed async methods for every §5.1–§5.7 and §5.9–§5.11 method, plus `futures::Stream` views over the event channel.
...
- §5.11 `accessibility_tree(AccessibilityTreeRequest) -> AccessibilityTreeResult{tree: AccessibleTree, text: String}`; `find_accessible(FindAccessibleRequest) -> FindAccessibleResult{window_id, matches: Vec<AccessibleMatch>, truncated}`; `invoke_accessible_action(InvokeAccessibleActionRequest) -> InvokeAccessibleActionResult{action_id, node_id, action}`. There are no accessibility events and no accessibility `EventKind`; observation is on demand only.
...
- §5.11 `AccessibilityTreeRequest::new()` (= `Default`) + `.window(WindowId)/.max_depth/.max_nodes/.include_bounds/.include_states/.include_actions/.include_text`; every field is `Option<..>` and omitted from the params when unset, so the server applies its §5.11 defaults (max_depth 12, max_nodes 2000, all four projection flags true, window resolved like an unscoped observation).
- §5.11 `FindAccessibleRequest::new()` (= `Default`) + `.window/.role/.name/.name_contains/.value_contains/.max_results`, all optional and AND-ed; `InvokeAccessibleActionRequest::new(node_id: AccessibleId)` + `.action(impl Into<String>)` (`action` omitted means the element's default action).
...
- `tests/api.rs` — one round-trip per §5.1–§5.7 and §5.9–§5.11 method: assert the exact request `method`/`params` JSON and that canned results deserialise into the typed values. The §5.9 tests cover a `Notification` with actions/hints; the §5.10 test asserts a `wait_for_events` response maps a `notification` record and an unknown-kind record onto typed `AgpEvent`s (with a `timed_out` result and empty events also covered); the §5.11 tests cover the `accessibility_tree` tree+outline round trip and its "omits unset optionals" client-level serialisation (`{}`), a `find_accessible` round trip with several AND-ed filters and a `truncated: true` result, `invoke_accessible_action` with and without an explicit action (the default-action response is returned verbatim), and `unknown_accessible`/`not_supported` surfacing as `ClientError::Server`.
...
- **Adding an AGP method group** (e.g. a new §5 section): add `src/api/<group>.rs` (module doc, `impl Client` block, typed request/result structs), register it in `src/api/mod.rs`, and re-export the public types from `src/lib.rs` next to the other `// --- AGP §5.x` lines. The pattern is mechanical — the §5.9 file `src/api/notification.rs` is the reference: public request structs are built by `new` + builder methods and `#[non_exhaustive]` when optionals exist; params/result envelopes the caller never names stay `private` (e.g. `ListNotificationsParams`); the method body is `self.request("<method>", &params).await`. `Client::request` (`src/client.rs:154`) is the only call point — `P = Serialize` params, `R = DeserializeOwned` result.
- **Adding a new event kind.** A core kind (defined in `adesk_core::RuntimeEvent`) needs no client-specific mapping beyond the `EventKind` enum: add the variant in `src/events.rs` (enum ~L42, `as_str`, `From<CoreEventKind>`), then the name in `kind_from_name` (L189) and — if `adesk_proto::EventKind` also has it — `wire.rs::event_name`. The `runtime_event` reconstruction (L297) is automatic. A protocol-only kind (like `quiet`) additionally needs an `AgpEvent` variant and an arm in `agp_event_from_raw` (L228), `event_kind` (L173) and `event_window_id` (L210). A kind this client does not model already surfaces as `AgpEvent::Other` via the lenient wire path — no code change needed for forward compatibility (§7).
...
esigned in parallel).
- `../adesk-agent/CONTEXT.md` — primary consumer of this API.
gent/CONTEXT.md` — primary consumer of this API.
