...
- **`default_socket_path` and the server disagree on the last fallback.** The client hardcodes `/tmp/adesk.sock` (`src/client.rs`), while `adesk_server::default_socket_path` uses `std::env::temp_dir().join("adesk.sock")` (`adesk-server/src/config.rs`); they diverge whenever `TMPDIR`/the platform temp dir is not `/tmp`. Real-runtime callers should pass an explicit path (`Client::connect`/`ConnectOptions`) or set `ADESK_SOCKET` instead of relying on the default.
