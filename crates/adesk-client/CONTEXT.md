...
- `tests/events.rs` — subscribe yields typed `RuntimeEvent`s; kind/window filtering is applied locally (including typed `quiet`); `subscribe_frames` surfaces `Quiet`/`InspectFrame`/`Other` with their `seq`/`ts_ms` pinned (a malformed `quiet` payload falls back to `Other` with name/seq/ts_ms/data preserved); `EventStream` skips quiet frames; `QuietEvent` serde round-trips; dropping a stream sends `unsubscribe_events`; lag yields `Lagged`; connection close ends the stream with `Closed`.
...
- `docs/protocol.md` §5.6 lists 11 `EventKind` values while `adesk_core::EventKind` has 9; the filter enum carries all 11.
  ...
  `EventKind::as_str` is a third hand-maintained copy of the wire names and the client enum omits `InspectFrame`, so an `inspect_frame` subscription filter cannot be expressed through the SDK (adding the variant is a public-API change, deliberately not made unilaterally).
