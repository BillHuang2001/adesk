...
- Clipboard rules (see `clipboard.rs` module docs): `wl_data_source.send` writes the stored bytes synchronously on the reader thread (test payloads must stay ≤ 64 KiB or the reader can stall on a full pipe); the read-to-EOF of a receive pipe runs on a short-lived worker thread awaited with `recv_timeout` (detached if the peer never closes — bounded by process exit); `set_selection`/`clear_selection` need a real input serial and wait for one, bounded; both only flush — acceptance is compositor-side (Smithay requires the client to hold keyboard focus, ignores the serial and drops the request silently otherwise), so `Ok(())` means "on the wire", never "accepted".
...
- **Released ranges are attributed by `wl_buffer` object id.** `ShmPool` holds a `live: HashMap<ObjectId, (offset, len)>` and the `wl_buffer.release` arm calls `ShmPool::release(&proxy.id())`, which moves the range to the free list (coalescing with touching neighbours) so a commit loop reuses one allocation. Slots cannot be used for this: the compositor releases a superseded buffer while dispatching the superseding commit (the window slot already holds the new buffer) and a destroyed window's slot is deregistered before its final release arrives. A release the table cannot attribute is a no-op.
## Notes for Agents
- `mod.rs` documents the reader-thread, lock-order (`ClientState` → `WindowState` → `ShmPool`) and teardown rules; `state.rs` documents configure sequencing and clipboard state; `input.rs` documents the recording contract; `clipboard.rs` documents serial handling, offer tracking and fd lifetimes; `shm.rs` documents the Argb8888 byte order.
