...
## Known Issues
- `adesk-compositor` never calls Smithay's `set_data_device_focus`, so **no client receives `data_offer`/`selection`** today: `read_selection*` cannot complete and reports the documented no-offer `Timeout`, and there is no same-client echo. The clipboard capstone test detects which behaviour is live and asserts the round trip automatically once the compositor wires data-device focus.
## Notes for Agents
