...
Status: implemented — `src/` and `tests/` contain no `todo!()`/`unimplemented!()` bodies, and the full `cargo test -p adesk-testkit` suite passes under the dev shell (last full run: 66 tests + 3 doctests, 0 failures; a source count finds 62 test functions — see Known Issues).
No behavioural skips: the only `ignore`d doc blocks are the 3 doc-code fences (`src/assert/event.rs:85`, `:203`, `src/assert/image.rs:45`).
...
- Run with `./scripts/dev.sh cargo test -p adesk-testkit`; `--all-features` is a no-op because the crate declares no `[features]` (`Cargo.toml`), and the only environment gate is the env var `ADESK_TEST_GL=1` (`src/gate.rs`) — in this crate it only flips `gl_enabled`/`require_gl`/`test_renderer`, no in-crate test starts a GL runtime.
- No test is `#[ignore]`d; the crate's `#[test]`/`#[tokio::test]` source count is 62, and the doc-fence inventory is 3 `no_run` (`src/lib.rs:24`, `src/gate.rs:27`, `src/fixtures/mod.rs:75`) + 3 `ignore` + 5 `text`.
...
- Test counts disagree between docs and source: this file and the root CONTEXT quote 66/49 tests for `--all-features`, while the source contains 62 `#[test]`/`#[tokio::test]` annotations (one per test function); re-run the suite to settle the count.
