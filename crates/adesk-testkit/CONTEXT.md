...
Status: implemented — `src/` and `tests/` contain no `todo!()`/`unimplemented!()` bodies; the suite is verified green by a full workspace run (66 tests + 3 doctests passed, 3 doctests ignored as doc-code fences, 0 failures).
