...
- `crop` and `downscale` (`src/image.rs`) index `image.data`/`out.data` by computed offsets and assume a well-formed `ImageBuffer` (i.e. `stride >= width * 4` and `data.len() >= stride * height`). Unlike `encode_png`/`tight_rgba`/`image_from_readback`, they do **not** validate the buffer length, so a hand-built or foreign `ImageBuffer` that under-states its data panics on an out-of-bounds slice instead of returning a `RenderError` (`ImageBuffer`'s fields are public). Every in-workspace producer emits a tight, correctly sized buffer, so the real callers (`render_scene`'s `downscale`, `adesk-inspector`'s `post.rs`) never hit it; `crop` is not on the readback path at all.
