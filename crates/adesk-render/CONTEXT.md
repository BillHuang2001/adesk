...
- The GL gate is skip-by-early-return, not `#[ignore]`: `gl_renderer()` returns `None` while the test still reports `ok` when `ADESK_TEST_GL != 1` or when EGL/GL/`GlesRenderbuffer` setup fails, and the skip reason only goes to `eprintln!` (captured by libtest, shown only with `--nocapture`). A GPU-less run with the gate on therefore shows 12 passed while exercising no GL, and adesk-render has zero `#[ignore]`d tests.
- No test has a wall-clock sleep, timeout or `Instant`/`Duration`/`thread` use; all four test targets finish in `0.00s` (whole suite ≈ 20 ms of runtime, dominated by process start).
- `import_buffer` has no caller at all (no production or test reference anywhere in the workspace; a direct test would also need a live wayland connection). End-to-end coverage stays in `adesk-compositor/tests` and `adesk-server/tests` via `adesk-testkit`.
