...
### Transport framing (`src/transport.rs`)- `DEFAULT_MAX_FRAME_LEN` (32 MiB) — the one shared line cap for server and client.
...
- `ViewerInput::CloseWindow` has no `adesk-server` counterpart yet: the runtime's `apply_input` must gain that arm (step 3 of the coordinated change) before `crates/adesk-server` compiles again. This crate is self-contained and green on its own.
