...
## Known Issues
- Reply FIFOs are aligned for sequential requests, but two *simultaneously in-flight* `id`-echoing requests on one connection are not reliably separated: an `error` is fanned out on the shared broadcast `errors` stream, so it can fail a sibling pending `request_state`/recording call, and a reply arriving before the failing awaiter removes its own waiter can be delivered to that stale slot. The crate's own callers issue these requests one at a time, so this is not exercised; fixing it needs a per-request error routing keyed by the echoed `id` (recording) rather than one broadcast stream.
