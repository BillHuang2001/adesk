...
- **A VAP `error` reply consumes its reply-FIFO entry.** `request_state` and the recording round trip register a waiter in their FIFO *before* writing the request, but a request can be answered by an `error` (delivered through the shared error broadcast stream) instead of the expected `state`/`recording` reply. Each registration carries a unique token, and the awaiting side removes its own entry from the FIFO on the error branch (and if the write itself fails), so no stale waiter is left at the head to swallow the next reply. Every `state`/`recording` reply (success) still pops the oldest waiter, keeping the FIFO aligned. Invariant: after any `request_state`/recording request completes — `Ok` or `Err` — no waiter for it remains. Regression guards: `tests/client.rs::a_rejected_recording_request_does_not_swallow_the_next_reply` and `a_failed_state_request_does_not_swallow_the_next_reply`.
