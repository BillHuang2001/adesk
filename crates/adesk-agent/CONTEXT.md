...
Tests: `./scripts/dev.sh cargo test -p adesk-agent --features test-support` = 103 passed / 0 failed / 0 ignored; `--features e2e` adds 14 real-runtime end-to-end tests (`tests/e2e_runtime.rs`), so `--features test-support,e2e` = 117. Default features run 81 (the loop, dummy-provider and scenario suites are feature-gated).
...
## Test Strategy
- Canonical commands: `./scripts/dev.sh cargo test -p adesk-agent` (81), `... --features test-support` (103), `... --features test-support,e2e` (117 = 103 + 14), `... --features e2e` (95 = 81 + 14 e2e).
- `--features test-support` = 103 tests: 63 lib unit, 2 `src/main.rs` binary, 10 `tests/agent_loop.rs`, 7 `tests/context_budget.rs`, 2 `tests/dummy_provider.rs`, 9 `tests/metrics.rs`, 10 `tests/scenarios.rs`; default features run 81 (the loop/dummy-provider/scenario suites are `#![cfg(feature = "test-support")]`).
