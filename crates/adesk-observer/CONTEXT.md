...
## Test Strategy
- Unit tests live inline in modules for the pure pieces (filters, condition predicates, journal `push`/`prune`/`since` + `max_seq` short-circuit, registry allocation, clock anchoring, service state machine, guards) — `#[cfg(test)]`, 62 total; tokio's paused clock is needed by the clock tests, the service's timer tests, and every service test that asserts a clock-derived value exactly (`now_ms()`, `record.ts_ms`, `snapshot().ts_ms`) — several of the state-machine tests run as `#[tokio::test(start_paused = true)]` for that reason, because the observer reads the *real* monotonic clock in a plain `#[test]` (`waiter.rs` and the rest are plain `#[test]`).
