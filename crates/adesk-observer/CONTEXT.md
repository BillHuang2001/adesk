...
## Test Strategy
- Unit tests live inline in modules for the pure pieces (filters, condition predicates, journal eviction, registry allocation, clock anchoring, service state machine) — `#[cfg(test)]`, 85 total; tokio's paused clock is needed only by the clock tests and the service's timer tests (`waiter.rs` and the rest are plain `#[test]`).
