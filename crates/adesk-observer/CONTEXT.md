...
- The `quiet` evidence flag for a non-quiet condition (`change`/`timeout`) is resolved against `ObserverConfig::default_quiet_ms`; non-quiet requests carry no per-request quiet threshold (`quiet_ms` exists only on `wait_for_quiet` and inside `Condition::Quiet { quiet_ms }`).
  ...
  That Quiet→own / Change|Timeout→constant mapping is written three times (`Condition::quiet_threshold_ms` in `src/spec.rs`, `WaitCondition::quiet_threshold_ms` in `src/waiter.rs` — whose non-Quiet arm is unreachable because `run_wait` matches on the condition first — and the inline match in `run_wait`, `src/service.rs`), so the three copies can drift apart silently.
