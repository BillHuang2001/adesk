...
## Test Strategy
- Unit tests live inline in modules for the pure pieces (filters, condition predicates, journal eviction + `since` short-circuit, registry allocation, clock anchoring, service state machine) — `#[cfg(test)]`, 90 total; tokio's paused clock is needed by the clock tests, the service's timer tests, and every service test that asserts a clock-derived value exactly (`now_ms()`, `record.ts_ms`, `snapshot().ts_ms`) — 6 of the state-machine tests run as `#[tokio::test(start_paused = true)]` for that reason, because the observer reads the *real* monotonic clock in a plain `#[test]` (`waiter.rs` and the rest are plain `#[test]`).
...
`ObserverService::window_state()` clones the whole `WindowTemporalState` (incl. `last_damage`) — `src/service.rs`; callers that only need geometry should use `ObserverService::window_geometry()`, which reads just the `Rect`. `adesk-server`'s event pump still calls `window_state()` once per `SurfaceCommit` just to read `geometry` (read-only sibling, out of scope here — the switch to `window_geometry()` is routed to the parent).
