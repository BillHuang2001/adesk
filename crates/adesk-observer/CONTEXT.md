...
- `Clock::deadline` (`src/clock.rs`) computes `anchor_instant + (ts - now_ms())` — it is early by the tokio time elapsed since the anchor was last re-anchored, where reaching `ts` needs `anchor_instant + (ts - anchor_ts_ms)` (equivalently `Instant::now() + (ts - now_ms())`). While a wait sleeps its anchor is not re-anchored, so once more than half the remaining event-domain time has passed the computed deadline lands in the past: `tokio::time::sleep_until` returns at once and `run_wait` re-evaluates in a spin (it drains the event clock only through timers, so it terminates in real time but hangs under `tokio::time::pause()`). Every current test re-anchors the clock immediately before the wait (an event at the wait's own `ts_ms`), so the offset is 0 and it stays latent.
