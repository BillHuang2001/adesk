...
- `new()`, `Default` (no configuration seam: the journal capacity and the quiet default come from the crate constants).
...
- Queries (sync, never wait): `snapshot() -> ObserverSnapshot`, `window_state(WindowId) -> Option<WindowTemporalState>`, `window_geometry(WindowId) -> Option<Rect>`, `watermark() -> u64`, `now_ms() -> u64`.
### Specs (`src/spec.rs`)
...
- There are zero `#[allow(dead_code)]` attributes in `src/`.
## Routing Table
| Area | Owner |
|---|---|
| Event ingestion, per-window state machine, watermark | `./src/service.rs` |
| Wait loop, `PendingGuard`, query methods (`window_state`/`window_geometry`) | `./src/service.rs` |
| Filters, accumulator, condition evaluation, `WaitPlan`, `WaitCondition::quiet_threshold_ms` | `./src/waiter.rs` |
| Action registry, `ActionKind`, `ActionRecord` | `./src/actions.rs` |
| Counted-event journal, `CountedEvent`/`CountedKind`, `since`/`max_seq` | `./src/journal.rs` |
...
- **`quiet` is an evidence flag, not a promise.** It is `now_ms - quiet_anchor >= threshold` at resolution, where `threshold` is the condition's `quiet_ms` for a quiet condition and the `DEFAULT_QUIET_MS` constant otherwise. The anchor is `max(plan.anchor_ts, last counted commit ts)` for condition evaluation and quiet-deadline scheduling (they must agree, or a quiet wait can wake early and never resolve); the resolve-time evidence flag uses `Accumulator::quiet_anchor(plan.anchor_ts)` (filter's last counted commit, else the plan anchor), i.e. the same anchor as the condition, so a condition-satisfied quiet wait always reports `quiet: true` (its flag is read at a clock value ≥ the condition's). `timed_out` is true only when the deadline expired before the condition was met. `Condition::Timeout` reaches its horizon by definition, so it reports `timed_out: false` and whatever accumulated (animation sampling).
- **Quiet-threshold mapping is single-sourced.** `WaitCondition::quiet_threshold_ms()` (`src/waiter.rs`, no parameter) maps `Quiet { quiet_ms }` → its own `quiet_ms` and `Change`/`Timeout` → `DEFAULT_QUIET_MS`; `run_wait` puts the result into `ResolveContext::quiet_threshold_ms`, which the resolve-time evidence flag reads.
...
- **Counted kinds.** `SurfaceCommit`, `WindowCreated`, `WindowDestroyed`, `WindowActivated`, `TitleChanged`, `FocusChanged`, `PopupAppeared`, `PopupDisappeared`. `AppLaunched` and the notification events (`Notification`, `NotificationClosed`, `NotificationAction`) are *not* counted (no window, no GUI state change); they only advance the watermark.
...
- **Queries**: window listing is *not* an observer query — `list_windows`/`get_window`/focus answers come from the compositor's `QueryState` reply (live geometry, focus, popups). The observer's sync queries feed the inspection frames: `snapshot()` picks the latest commit/damage (`adesk-server/src/inspection.rs`) and `window_state()` supplies action-marker geometry (`inspection.rs`, `event_pump.rs`). `window_geometry()`, `watermark()`, `now_ms()` and `snapshot().journal_len` have no server call site in v1 (`window_geometry()` is the cheaper replacement for the `window_state()`-for-geometry call, still routed to the parent).
...
## Test Strategy
- Unit tests live inline in modules for the pure pieces (filters, condition predicates, journal `push`/`prune`/`since` + `max_seq` short-circuit, registry allocation, clock anchoring, service state machine, guards) — `#[cfg(test)]`, 64 total; tokio's paused clock is needed by the clock tests, the service's timer tests, and every service test that asserts a clock-derived value exactly (`now_ms()`, `record.ts_ms`, `snapshot().ts_ms`) — several of the state-machine tests run as `#[tokio::test(start_paused = true)]` for that reason, because the observer reads the *real* monotonic clock in a plain `#[test]` (`waiter.rs` and the rest are plain `#[test]`).
...
- The inline suite and the `./tests/*.rs` specs now overlap only lightly: the inline module owns the state machine, resync synthesis, guard bookkeeping, journal/clock primitives and the pure waiter pieces, while the frozen specs are the protocol acceptance layer. Exactly one pair shares a name: `record_action_captures_watermark_and_clock` (`./tests/actions.rs` ↔ inline `src/service.rs`); the inline copy pins the clock via `#[tokio::test(start_paused = true)]`, while the frozen copy bounds its real-clock reads (`before <= record.ts_ms <= after`). See `./tests/CONTEXT.md` for the duplication map, the fixture-builder duplication forced by Rust test-module isolation, and the fact that no test uses a wall-clock sleep.
## Known Issues
...
- Non-quiet requests (`change`/`timeout`) carry no per-request quiet threshold (`quiet_ms` exists only on `wait_for_quiet` and inside `Condition::Quiet { quiet_ms }`), so their `quiet` evidence flag is resolved against the `DEFAULT_QUIET_MS` constant.
...
- Public API with no consumer outside this crate (documented surface reached only by in-crate tests): `ObserverService::{action, action_seq, watermark, now_ms, window_geometry}`, `ObserverSnapshot::{seq, ts_ms, actions_len, journal_len, events_dropped}`, `ActionRegistry::{contains, last, is_empty}`, and `pending_observation`, which is maintained and tested but rendered by nothing.
...
- `Clock::deadline` (`src/clock.rs`) computes `anchor_instant + (ts - now_ms())` — it is early by the tokio time elapsed since the anchor was last re-anchored, where reaching `ts` needs `anchor_instant + (ts - anchor_ts_ms)` (equivalently `Instant::now() + (ts - now_ms())`). While a wait sleeps its anchor is not re-anchored, so once more than half the remaining event-domain time has passed the computed deadline lands in the past: `tokio::time::sleep_until` returns at once and `run_wait` re-evaluates in a spin (it drains the event clock only through timers, so it terminates in real time but hangs under `tokio::time::pause()`). Every current test re-anchors the clock immediately before the wait (an event at the wait's own `ts_ms`), so the offset is 0 and it stays latent.- Popups have no state of their own: `PopupAppeared`/`PopupDisappeared` are owner-window counted events and `WindowSnapshot::popup_count` is accepted by `resync` but ignored.
...
## Notes for Agents
- `src/service.rs` is ~810 production lines plus a ~750-line cohesive inline test module; the single-file layout is deliberate (one state machine, one lock, tests next to the code they pin). Do not split it to satisfy the ~1000-line soft threshold.
...
- `Observation::quiet` is the evidence flag above, *not* "condition met": it reports quiet for the applicable threshold (`Condition::Quiet`'s own `quiet_ms`, otherwise the `DEFAULT_QUIET_MS` constant) at resolution time, as `docs/protocol.md` §5.4 defines.
  A timed-out `wait_for_change` can legitimately carry `quiet: true` (`DEFAULT_QUIET_MS` = 250 ms evidence threshold).
...
- `ObserverService::window_state()` clones the whole `WindowTemporalState` (including `last_damage`); geometry-only callers should use `ObserverService::window_geometry()`, which clones just the `Rect`.
...
- Every `SurfaceCommit` clones its damage `Region` twice: once into the journal `CountedEvent` (`src/journal.rs`) and once into `WindowTemporalState::last_damage` (`src/service.rs`); `last_damage` is read only by the inspection path (`adesk-server/src/inspection.rs`).
- Three lock acquisitions per event: clock mutex (`observe_ts`), state mutex, and the `watch` internal lock in `send_modify` (`src/service.rs`).
- The `watch<u64>` generation is global: every event wakes *every* active waiter, regardless of its window filter.
- `EventJournal::push` raises the running `max_seq` and performs at most one eviction per append (a single `if`), independent of capacity.
Per-wakeup waiter path — `run_wait` (`src/service.rs`):
- Each wakeup consults `journal.since(cursor)`. The journal's running `max_seq` (raised on push, never lowered) makes `since` an O(1) empty iterator when the cursor is at or above it (the idle case: no new event since the last absorb). This stays correct despite the post-`resync` non-seq-monotonic buffer.
- `Accumulator::absorb` folds counted commit damage rect-by-rect with a per-rect containment check: in-bounds rects extend the union (`self.damage.push`) directly and only rects that escape the window geometry are clipped (`intersect`) — no temporary clipped `Region` on the common path. Unknown geometry leaves damage unclipped; an empty geometry drops all damage. Semantics are identical to folding `damage.clip(&geometry)`.
- `Accumulator::damage` still accumulates without dedup/coalescing during a wait, and `resolve(&mut self)` consumes it via `mem::take(&mut self.damage).simplified()` — `Region::simplified()` → `coalesce()` is an O(n²) pairwise merge over that union.
Journal ordering gotcha: the buffer is NOT seq-monotonic after `resync` — `prune_through(snapshot.seq)` keeps only `seq > snapshot.seq` (`src/service.rs`) and the synthetic events are then pushed at the *tail* with `seq == snapshot.seq` (`src/service.rs`), so binary search or backward scans over the buffer are unsafe.
`adesk-server`'s event pump still calls `window_state()` once per `SurfaceCommit` just to read `geometry` (read-only sibling, out of scope here — the switch to `window_geometry()` is routed to the parent).
## Dependencies
