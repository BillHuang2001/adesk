...
Every module is implemented (`src/` has no `todo!()`); `cargo test -p adesk-server` is green — 189 tests across lib, bin and the nine integration targets (129 lib + 4 bin + 56 integration), 0 failed, 0 ignored.
