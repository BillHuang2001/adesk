...
- **An `inspect_subscribe` stream that stops cannot say so on the wire.** §5.7 defines no termination event: when the push loop ends (render failure, `unsubscribe_events`, a closed connection) the client observes the `inspect_frame` stream simply stop, and its `inspect_subscribe` response is long since sent. The runtime side stays coherent — the loop deregisters its own `InspectRegistry` entry before returning (`dispatch/inspect.rs::inspect_loop`) — so nothing stale claims a live stream; only the protocol has no message for it.
