...
Every module is implemented (`src/` has no `todo!()`); `cargo test -p adesk-server` is green — 188 tests across lib, bin and the nine integration targets (129 lib + 4 bin + 55 integration), 0 failed, 0 ignored.
...
- **An `inspect_subscribe` push loop that exits on a render/encode error leaves its `InspectRegistry` entry registered.** `dispatch/inspect.rs::inspect_loop` returns on `Err` after a `debug!` log — no retry, no client notification — while `event_pump::prune_inspect_streams` drops only streams whose sink is closed and the loop's own `subscription_alive` check dies with the loop; the entry survives until `unsubscribe_events` or the connection closes. Reachable only when the compositor/encoder fails (typically shutdown), so the impact is a stale registry entry and a silently stopped stream, not a wrong response.
- **`docs/protocol.md` §4's "no window renderable ⇒ `image: null`" clause has no implementation.** With `include_image=true`, `observation_image` returns `Ok(None)` (→ `"image": null`) only when `observed_window` finds no candidate at all — the observation's `window_id` is `None` and both `snapshot.active_window_id` and `snapshot.keyboard_focus` are `None` (`dispatch/capture.rs:186-189`, `:206-215`). A named window that fails to render propagates `Err` (`capture.rs:191`, no `ok()`/`unwrap_or`/`transpose`) into an AGP error response (`render_failed`/`unknown_window`/`shutting_down`/`internal` via `windows.rs::command_error` → `dispatch/mod.rs:88`), and a tracked window whose surface currently has no buffer renders Ok as a full-size clear frame (compositor + `adesk-render` have no empty-scene error), i.e. a non-null image; a toplevel that never committed a buffer is not even a window (no `WindowId`), so it cannot be the target. `docs/` owns the statement; the server must not silently diverge if the doc is tightened.
...
- **E2E level (`./tests/`, nine integration targets, 55 tests):** `common::TestRuntime` starts one real runtime per test (`Server::start`) on a private temp socket with the pixman renderer, driven through `adesk-client` and the raw NDJSON `RawClient`; `sequence.rs` brackets both server-synthesized `seq` emission sites with out-of-band `ReserveSeq` probes to prove the single global `seq` domain; full plan in `./tests/CONTEXT.md`. No test may require a display, GPU, network or installed application (`subscriptions.rs`'s launch fixture spawns `true` from `PATH`).
- **No E2E sets `include_image=true`** (every observation test passes `include_image(false)` or uses the waits, whose default is `false`), so the runtime's `image: null`-with-image-requested path is not covered end-to-end here; only the wire shape is pinned, by the unit test `translate::tests::observe_result_serializes_as_a_nested_observation_with_null_image`.
