...
- **The event pump is the matching half of the launch → window correlation.** `launch_app` records the `LaunchRecord`; `event_pump::correlate_window` feeds a `WindowCreated` with `launch_id: None` to `Correlator::correlate` as a `WindowCandidate` (window id, pid, app_id, title) and hands the *same* stamped event to the observer, the inspection cache and the fan-out, so all three agree. It returns a `Cow`: only a correlated `WindowCreated` is cloned. Uncorrelated windows pass through untouched (never guessed), events that already carry a `launch_id` are never re-correlated, and the lock is poison-recovering. This projects a server-side verdict onto the AGP/observer view; it cannot change what the compositor already published on its own broadcast (see Known Issues).
...
- **`WindowCreated.launch_id` is `None` on the compositor's own broadcast.** `WmBridge::note_launch` (`crates/adesk-compositor/src/wm.rs`) is kept under `#[allow(dead_code)]` and no `RuntimeCommand` feeds it, so the compositor's `LaunchLedger` is always empty and it emits every mapping with `launch_id: None`; the server can only stamp the events it projects (observer, inspection, fan-out — see Design Decisions). `adesk-testkit`'s `EventAssert::tap` subscribes to that raw broadcast (`TestRuntime::event_tap` → `CompositorHandle::subscribe`), so `fixtures::launch_app_starts_helper_window` still fails at its `launch_id` assertion. Closing it needs a compositor-side change (a `RuntimeCommand` variant feeding `note_launch`, plus `launch_app` sending it) — outside this node, flagged to the parent; `tests/subscriptions.rs::window_created_event_carries_the_correlated_launch_id` pins the server-side half and documents the split.
