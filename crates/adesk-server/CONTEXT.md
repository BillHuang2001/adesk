...
Every module is implemented (`src/` has no `todo!()`); `cargo test -p adesk-server` is green — 213 tests across lib, bin and the ten integration targets (139 lib + 9 bin + 65 integration), 0 failed, 0 ignored.
