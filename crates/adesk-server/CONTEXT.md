...
Phase 2: every module is implemented (`src/` has no `todo!()`), the public API is unchanged from Phase 1, `cargo test -p adesk-server --lib` is green (129 tests), and the E2E suites in `./tests/` are green as well (188 tests across lib, bin and the nine integration targets, 0 failed, 0 ignored).
...
- **Server-synthesized events reserve their `seq` from the compositor.** `dispatch::windows::reserve_seq(&ServerContext)` sends `RuntimeCommand::ReserveSeq` and returns the next number from the compositor's central counter without emitting an event; it is the only `seq` source for `launch_app`'s `AppLaunched` and each `inspect_frame` push (one reservation per emission — the `QueryState` watermark is never used as a stamp). This keeps a single global monotonic `seq` domain across compositor- and server-emitted events (protocol §1): gaps are allowed (spawn failure, dropped/throttled frame), reuse is not. `ts_ms` keeps its existing source at both sites (compositor snapshot clock for `AppLaunched`, inspection snapshot for the frame).
