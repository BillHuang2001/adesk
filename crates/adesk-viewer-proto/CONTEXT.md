...
- `docs/viewer.md` §3's `state` example shows a `focus` field; the crate intentionally omits it (see Design Decisions) — resolve any §3/§4 tension in favour of this file and the `adesk-viewer` client.
- `ViewerHello.client` is `#[serde(default)]`; every other handshake/frame/state field is required on the wire.
## Status
