...
- **App management is additive too (§7).** `list_apps`/`launch_app`/`close_window` and the server `apps`/`launch_result` replies are additive to VAP v1 (`PROTOCOL_VERSION` stays 1). Every reply echoes the client `id` when the request carried one. `list_apps.query` is optional and omitted from the wire form when `None`; on `apps` the `apps` list is required and `id` is omitted when `None`. `launch_result` **flattens** its [`LaunchOutcome`] (via `#[serde(flatten)]` on the decode-side `LaunchResultFields` and a manual `map.extend` on the encode side, exactly like `recording`'s status), so `app_id`/`launch_id` (required) and `action_id`/`window_id` (omitted when `None`) sit at the top level beside `id`. `close_window` is a flat object carrying no `id`.
## Test Strategy
