...
/// Draws `"app {app_id}"` in slot 1 of every window's top-left corner (slot
/// layout is shared in `paint::labels`) in input order; windows without an app
/// id draw `"app ?"` (correlation failure is reported, never guessed).
///
