...
- Clipboard publications in `tests/clipboard.rs` are ordered against a following `ActivateWindow` only by "requests flushed to the socket before the command was queued": the wayland display fd and the command channel are independent calloop sources, and `WaylandTestClient::roundtrip()` is a flush plus one reader poll cycle, not a `wl_display.sync` barrier, so it does not prove the compositor dispatched a preceding `set_selection`. If the focus change is serviced first, Smithay silently drops the publication: the reader then sees zero offers (first publication) or a stale offer (supersede) — a loud bounded-timeout/payload assertion, never a silent pass; the `publish` doc comment claiming a "sync callback" barrier is inaccurate. The ordering barrier that closes this is a commit on the publisher's own connection (`commit_pending()` + awaiting that surface's `RuntimeEvent::SurfaceCommit`), as used by `adesk-testkit`'s `tests/clipboard.rs`.
- Stale comment wording only (no code defect): the `src/render/elements.rs` and `src/render/headless.rs` test modules still call their popup and tracked-but-inactive pixel proofs "queued" `tests/integration_plan.md` scenarios, but both are implemented (`tests/popups.rs`, `tests/output_composition.rs`); the plan doc is the normative statement.
