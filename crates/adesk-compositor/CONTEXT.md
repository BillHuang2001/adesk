...
- Clipboard publications are ordered against a following `ActivateWindow` only by "requests flushed to the socket before the command was queued": the Wayland display fd and the command channel are independent calloop sources, and `WaylandTestClient::roundtrip()` is a flush plus one reader poll cycle, not a `wl_display.sync` barrier, so it does not prove the compositor dispatched a preceding `set_selection`. If the focus change is serviced first, Smithay silently drops the publication: the reader then sees zero offers (first publication) or a stale offer (supersede) — a loud bounded-timeout/payload assertion, never a silent pass; the `publish` doc comment in `tests/clipboard.rs` claiming a "sync callback" barrier is inaccurate.
