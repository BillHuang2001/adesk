...
## API Surface
Crate root (`src/lib.rs`) glob-re-exports every public item of every module (`adesk_machine::<Name>`); `sync` is a private internal module.
### Errors (`src/error.rs`)
...
- `MachineStatus { id, name, image, state, pid: Option<u32>, created_at_ms: u64, viewer }` + `is_running()`.
### Backend seam (`src/runtime.rs`, `src/runtime/`)
- `#[async_trait] trait ContainerRuntime: Send + Sync + 'static` — `create(&MachineSpec) -> Result<MachineId>`, `start(&MachineId)`, `stop(&MachineId, timeout_ms)`, `remove(&MachineId, force)`, `status(&MachineId) -> Result<MachineStatus>`, `list() -> Result<Vec<MachineStatus>>`.
- `PodmanRuntime` (`src/runtime/podman.rs`) — rootless Podman via `tokio::process::Command`: `new()` (uses `podman` from PATH), `with_program(path)` (drives a stub in tests), `with_binary_env(key, value)`; maps `podman create/start/stop/rm/inspect` onto the trait; pure `pub fn build_create_argv(&MachineSpec) -> Vec<String>` translates the whole spec (mounts, env, labels, memory/cpus, network, viewer exposure, image, command).
- `MockRuntime` (`src/runtime/mock.rs`) — in-memory, deterministic; models the lifecycle state machine (ids `machine-1`, `machine-2`, …), fake pids `1000 + seq`, records every call (`calls() -> Vec<RuntimeCall>`), `with_clock(Arc<dyn Fn() -> u64 + Send + Sync>)` for deterministic timestamps. `RuntimeCall` is one variant per trait method (`Create { name }`, `Start { id }`, `Stop { id, timeout_ms }`, `Remove { id, force }`, `Status { id }`, `List`).
...
- `MachineRegistry` — the name ↔ id map + last known `MachineStatus` cache; `new`, `len`, `is_empty`, `contains`, `id`, `get`, `insert`, `update`, `remove`.
...
- `ApprovalRequest { id: u64, machine: MachineName, capability: String, description: String }` (`new`), `ApprovalDecision { Allow, Deny { reason } }`, `#[async_trait] trait Approver: Send + Sync { async fn decide(&self, &ApprovalRequest) -> ApprovalDecision; }`, `ApprovalPolicy` (`new`, `pre_approve`, `is_pre_approved` — host-side pre-authorization that short-circuits the approver), `ApprovalRouter` (`new(Arc<dyn Approver>)`, `with_policy`, `policy`, `request(req)`); `AutoApprove`/`AutoDeny` approvers.
### Internal (`src/sync.rs`)
- `pub(crate) fn lock_unpoisoned<T>(&Mutex<T>) -> MutexGuard<'_, T>` — the single place the poisoned-lock recovery idiom lives; used by `MachineManager::lock_registry` and `MockRuntime::lock`.
### Binary (`src/main.rs`, ~553 lines)
...
- All subcommands drive the `HostControlPlane` (`restart` goes through `manager()`); `create` builds the spec from flags via pure `parse_mount`/`parse_viewer_unix`/`parse_viewer_tcp`/`parse_network`/`build_spec` helpers (`parse_viewer_unix`/`parse_viewer_tcp` share one `split_host_and_container` helper).
...
- `#![forbid(unsafe_code)]` and `#![deny(missing_docs)]`; files stay well under the ~1000-line threshold (largest: `runtime/podman.rs` 605, `main.rs` 553, `runtime/mock.rs` 444); split along module boundaries.
...
| `ContainerRuntime` trait | `./src/runtime.rs` |
...
| Shared poisoned-lock helper | `./src/sync.rs` |
...
- **`MockRuntime` is first-class.** Because CI has no container engine, the mock is the reference implementation of the lifecycle state machine; both the integration suites and the manager/host unit tests drive it (`MachineManager<MockRuntime>`). `PodmanRuntime` is verified against a stub program for argv/spec translation and error mapping.
...
- **Lock recovery lives once.** `sync::lock_unpoisoned` recovers the data from a poisoned `Mutex` instead of propagating a panic; both the manager's registry lock and the mock's interior lock use it.
## Test Strategy
Runs with `./scripts/dev.sh cargo test -p adesk-machine` → 84 passed, 0 failed (49 lib unit + 11 bin unit + 24 integration), no container engine/display/GPU/network.
- In-module unit tests cover the model/registry/approval, the `MockRuntime` lifecycle + recorded calls, the `PodmanRuntime` argv/inspect parsing + error mapping, and the CLI flag parsers.
- `./tests/lifecycle.rs` is the authoritative manager suite (create/start/stop/restart/remove, duplicate names, unknown names, illegal transitions as a table-driven state-machine script, `list()` cache rebuild, recorded call order).
- `./tests/spec_translation.rs` is the authoritative `build_create_argv` suite (default + fully-populated spec, each `NetworkMode`, viewer exposure → `--volume`/`--publish`, arg ordering).
- `./tests/approvals.rs` — `AutoApprove`/`AutoDeny`, pre-approval short-circuit, `HostCapabilities` mount/port matching, `HostControlPlane` delegation + `request_approval`.
...
- The integration suites are authoritative; the lib tests hold only white-box/unique cases (e.g. registry-cache freshness, the runtime-side duplicate path, `MachineError::Spawn`, default `AutoDeny` reason, serde round-trips) rather than duplicating integration coverage.
## Notes for Agents
...
- Process-spawning tests must fully write+close a stub script before spawning and serialize spawning with a process-wide mutex (a `tokio::sync::Mutex` `SPAWN_LOCK` static, which cannot be shared between the lib and an integration test because integration tests compile the lib without `cfg(test)`): writing then immediately exec'ing a just-written script can hit `ETXTBSY` ("Text file busy").
- `adesk-machine` declares no Cargo features; `MockRuntime` is always compiled and is the double for both unit and integration tests (there is no `#[cfg(test)]` runtime stub).
## Known Issues
...
- `docs/machine.md` (line ~44) still sketches `fn kind(&self) -> RuntimeKind;` on the `ContainerRuntime` trait; this crate no longer has `kind`/`RuntimeKind` (removed as dead code). The doc sketch is owned by `./docs/` — outside this node — and needs a follow-up edit there.
## Status
Implementation-complete and green: `error`, `spec`, `state`, `runtime` (trait), `runtime::mock`, `runtime::podman`, `registry`, `manager`, `host`, `approval`, `sync` and the `adesk-machine` CLI are all implemented, documented and tested; `src/lib.rs` re-exports the whole surface at the crate root.
Dead code removed: `RuntimeKind` + `ContainerRuntime::kind`, `ApprovalDecision::is_allowed`, `AutoDeny::reason`, `MachineSpec::with_name`/`with_image`, `NetworkMode::as_str`, `MachineRegistry::names`/`list`, and `MachineId`/`MachineName`'s `as_str`/`AsRef`/`From<..> for String`; `MachineStatus::is_running` was kept (used by the integration suite).
Deduplication: the `#[cfg(test)] StubRuntime` double was deleted in favour of the production `MockRuntime`; the poisoned-lock idiom now lives once in `sync::lock_unpoisoned`; the duplicated argv/stub/manager/host/approval inline tests were pruned (integration suites authoritative); `parse_viewer_unix`/`parse_viewer_tcp` share a split helper.
`cargo test -p adesk-machine` = 84 passed / 0 failed; `cargo clippy -p adesk-machine --all-targets --no-deps -- -D warnings`, `cargo fmt -p adesk-machine --check`, `cargo doc -p adesk-machine --no-deps --document-private-items` (zero warnings) and `cargo check --workspace --all-targets` are all clean.
