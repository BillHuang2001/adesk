...
`cargo test --workspace --no-fail-fast` = 1916 passed, 0 failed, 5 ignored; the 5 ignored are doc-code fences only.
...
The compositor's DMA-BUF path is hardened: every incoming dmabuf is validated before import, failures log the buffer geometry, and the `zwp_linux_dmabuf_v1` global is advertised only for a hardware GL renderer — a default (GPU-less) `adesk-server` demotes Mesa llvmpipe to pixman and runs SHM-only, so no default configuration reaches Mesa's software-GL raster fault or kills a GTK4 client with a fatal `create_immed` import failure (`crates/adesk-compositor/CONTEXT.md`).
That SHM-only fallback is proven end to end: a real GTK4 app (`gnome-calculator`) launched through the server's own AGP `launch_app` stays alive beyond 26 s with zero `dmabuf import failed` WARNs and renders a tiled `Calculator` toplevel (captured frame 56 KB / 419 colours vs a 22 KB blank baseline), with `DISPLAY`/session-bus isolation keeping the child off the host session.
